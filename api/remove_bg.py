...
app = Flask(__name__)
# Enforce a file size limit (10 MB) to protect serverless execution time & memory
...
def handler():
    """Vercel will mount this Flask `app` as a WSGI app.
    ...
    Error responses are JSON with `error` and (optionally) `details`.
    """
    ...
    try:
        for f in files:
            ...
            # Use rembg to remove background. rembg.remove accepts bytes and returns PNG bytes.
            try:
...
# make app variable available for WSGI
# Vercel's Python runtime will detect the Flask WSGI app via the `app` name
